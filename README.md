...
Edit `devices.yaml`. Every entry needs the manufacturer spec page as `source`. Body width, height and depth in mm, weight in grams, and the screen as pixels plus ppi in the same orientation as the body. Foldables list one state per configuration and can set `hinge` to draw a fold line. See the comments at the top of the file.
...
Pull requests for other makers are welcome. Samsung, Google and Nothing all publish the required numbers.
